...
for (const mode of ["melodic", "diatonic", "jazz"]) {
  test(`${mode} produit 3 à 10 notes dans les deux octaves`, () => {
    for (const length of [3, 5, 10]) {
      const result = makeSequence({ length, mode, random: seededRandom(length) });
...
test("un extrait Parker conserve une source précise", () => {
  const result = makeSequence({ length: 7, mode: "jazz", random: seededRandom(7) });
...
test("la transposition Parker est chromatique et reste dans le clavier", () => {
  const results = Array.from({ length: 16 }, (_, index) =>
    makeSequence({ length: 8, mode: "jazz", random: seededRandom(index + 20) }),
  );
...
test("la longueur est bornée", () => {
  assert.equal(makeSequence({ length: 1, random: seededRandom() }).notes.length, 3);
  assert.equal(makeSequence({ length: 99, random: seededRandom() }).notes.length, 10);
});
